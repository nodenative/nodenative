#ifndef _NATIVE_URITEMPLATE_H_
#define _NATIVE_URITEMPLATE_H_
...
namespace native
{
    ...
    public:
         ...
         * @param iForExtraction default false. if true, return regex pattern with capturing groups, false otherwise (to speed-up match operations).
         * @param iAddEndAnchor default true. add end anchor regex flag ("$").
         * @return regex pattern string
         * @note regex strings are initiated in the constructor
         */
        std::string getPattern(const bool iForExtraction = false, const bool iAddEndAnchor = true) const { return (iForExtraction ? _extractPattern : _matchPattern) + (iAddEndAnchor ? "$": ""); };
