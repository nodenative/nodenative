...
public:
  Stream() = delete;
  Stream(std::shared_ptr<Loop> iLoop) : Handle(iLoop), _maxAllocSize(0) {}
  ...
  void setMaxAllocSize(size_t iSize) { _maxAllocSize = iSize; }

  std::shared_ptr<Stream> getInstanceStream() { return std::static_pointer_cast<Stream>(Handle::getInstanceHandle()); }
