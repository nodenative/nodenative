#ifndef __NATIVE_EVENTS_HPP__
#define __NATIVE_EVENTS_HPP__
...
template <typename... M> class EventEmitter {
  ...
  template <typename E> struct callback_type {
...
public:
  ...
  template <typename E> auto addListener(typename callback_type<E>::type callback) -> decltype(&callback) {
  ...
  template <typename E> bool removeListener(typename callback_type<E>::type *callback_ptr) {
    auto &entry = std::get<event_idx<E>::value>(set_);
  ...
  template <typename E> void removeAllListeners() {
    auto &entry = std::get<event_idx<E>::value>(set_);
    entry.clear();
  }

  template <typename E, typename... A> void emit(A &&... args) {
    auto &entry = std::get<event_idx<E>::value>(set_);
    for (auto x : entry) {
      try {
...
private:
  ...
  template <typename... T> struct set_t<std::tuple<T...>> {
