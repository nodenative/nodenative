...
/** Enqueue a thread/parallel task callback. After the completion. The returned value will be enqueued in the current event Loop.
 *
...
worker(F&& f, Args&&... args) {
    NNATIVE_FCALL();
    std::shared_ptr<Loop> iLoop = Loop::GetInstanceOrCreateDefault();
...
worker(std::shared_ptr<Loop> iLoop, F&& f, Args&&... args) {
    ...
    std::shared_ptr<WorkerCallback<return_type, Args...>> action = WorkerCallback<return_type, Args...>::Create(iLoop, std::forward<F>(f), std::forward<Args>(args)...);
    action->resolve();
