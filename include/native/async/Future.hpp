#ifndef __NATIVE_ASYNC_FEATURE_HPP__
#define __NATIVE_ASYNC_FEATURE_HPP__
...
public:
   ...
   */
  Future(std::shared_ptr<Loop> iLoop) : _p(FutureShared<R>::Create(iLoop)) {}
   ...
   * @param f waiter (function or function object) callback, F(R, Args...). First argument is reserver for the current
   * result value, R.
  ...
  then(F &&f, Args &&... args) {
    return Future<typename ActionCallbackP1<typename std::result_of<F(R, Args...)>::type, R, Args...>::ResultType>(
        _p->template then<F, Args...>(std::forward<F>(f), std::forward<Args>(args)...));
  }
   ...
   */
  template <class F, typename... Args> Future<R> error(F &&f, Args &&... args) {
...
#endif // __NATIVE_ASYNC_FEATURE_HPP__
