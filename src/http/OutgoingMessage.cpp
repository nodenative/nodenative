...
std::string OutgoingMessage::getMessageHeaderRaw() const {
    ...
    // Add for get/set version
    message_raw << "HTTP/1.1 ";
    message_raw << _statusCode << " " << GetStatusText(_statusCode) << "\r\n";
    for(auto h : _headers)
    {
...
Future<void> OutgoingMessage::write(const std::string& str) {
    ...
    if(!str.empty()) {
        ...
        future = future.then([messagePtr, str]() {
            return messagePtr->send(str);
        });
    }
...
Future<void> OutgoingMessage::end(const std::string& str) {
    NNATIVE_ASSERT(!_closed);
...
Future<void> OutgoingMessage::end() {
    std::string emptyStr;
    return end(emptyStr);
}

void OutgoingMessage::setStatus(int status_code) {
    _statusCode = status_code;
}

void OutgoingMessage::setHeader(const std::string& key, const std::string& value) {
    _headers[key] = value;
}

std::string OutgoingMessage::GetStatusText(int status) {
    switch(status) {
