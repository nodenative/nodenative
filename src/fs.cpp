...
{
    UV_FS_METHOD_SYNCCALL_VOID(close, UV_FS_CLOSE, fd);
}
...
{
    UV_FS_METHOD_SYNCCALL_VOID(unlink, UV_FS_UNLINK, path.c_str());
}
...
Future<void> fchmod(const file_handle fd, int mode)
{
    UV_FS_METHOD_CALL_PROMISE_VOID(fchmod, UV_FS_CHMOD, fd, mode);
}

void fchmodSync(const file_handle fd, int mode)
{
    UV_FS_METHOD_SYNCCALL_VOID(fchmod, UV_FS_CHMOD, fd, mode);
}
