...
UriTemplateFormat::UriTemplateFormat(const std::string &iName, const std::string &iTemplate)
    : _name(iName), _template(iTemplate) {
  // No needs for cyclic dependency check, because at each UriTemplate constructor the dependency is checked and added
  // in series
}

UriTemplateFormat::UriTemplateFormat(const UriTemplateFormat &iOther)
    : _name(iOther._name), _template(iOther._template) {}
