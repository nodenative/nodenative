...
void Timer::init(uv_handle_t *iHandle) { base::Handle::init(iHandle); }
...
Error Timer::stop() { return Error(uv_timer_stop(&_uvTimer)); }
...
uint64_t Timer::getRepeat() { return uv_timer_get_repeat(&_uvTimer); }

void Timer::setRepeat(const uint64_t iRepeat) { uv_timer_set_repeat(&_uvTimer, iRepeat); }

Error Timer::again() { return Error(uv_timer_again(&_uvTimer)); }

Future<std::shared_ptr<Timer>> Timer::close() {
  return base::Handle::getInstanceHandle()->close().then(
      [](std::shared_ptr<base::Handle> iHandle) -> std::shared_ptr<Timer> {
...
void Timer::onClose(const std::function<void(std::shared_ptr<Timer>)> iOnClose) { _onClose = iOnClose; }
