  ...
  int res = uv_read_start(get<uv_stream_t>(),
                          ...
                          [](uv_stream_t *s, ssize_t nread, const uv_buf_t *buf) {
                            ...
                            if (nread < 0) {
                              ...
                              NNATIVE_ASSERT_MSG(nread == UV_EOF, "error nread: " << nread);
...
void Stream::AfterWrite(uv_write_t *req, int status) {
  std::unique_ptr<RequestPromiseInstance<void, uv_write_t, Stream>> reqInstance(
      static_cast<RequestPromiseInstance<void, uv_write_t, Stream> *>(req->data));
...
Future<void> Stream::shutdown() {
  std::unique_ptr<RequestPromiseInstance<void, uv_shutdown_t, Stream>> reqInstance(
      new RequestPromiseInstance<void, uv_shutdown_t, Stream>(_loop, getInstanceStream()));

  const int res = uv_shutdown(&reqInstance->_req, get<uv_stream_t>(), [](uv_shutdown_t *req, int status) {
    std::unique_ptr<RequestPromiseInstance<void, uv_shutdown_t, Stream>> reqInstance(
        static_cast<RequestPromiseInstance<void, uv_shutdown_t, Stream> *>(req->data));
    if (0 == status) {
      reqInstance->_promise.resolve();
    } else {
