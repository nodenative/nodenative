...
Handle::Handle(std::shared_ptr<native::Loop> iLoop)
    : _uvHandlePtr(nullptr), _loop(iLoop), _instanceHandleCount(0), _closingPromise(iLoop) {
  NNATIVE_FCALL();
  NNATIVE_CHECK_LOOP_THREAD(_loop);
}

Handle::~Handle() { NNATIVE_FCALL(); }

std::shared_ptr<Handle> Handle::getInstanceHandle() { return shared_from_this(); }

bool Handle::isActive() { return uv_is_active(get()) != 0; }

bool Handle::isClosing() { return uv_is_closing(get()) != 0; }

void Handle::ref() { uv_ref(get()); }

void Handle::unref() { uv_unref(get()); }

bool Handle::hasRef() { return uv_has_ref(get()) != 0; }
